...
// 降采样是 DOM(canvas) 依赖、jsdom 里图片不解码会挂起——mock 成"降采样为 JPEG"（生产即把 PNG/大图降成 JPEG），
// 让本文件专测 VQA 接线（且断言 mime=jpeg 可验证降采样确在调用链上）；实际降采样逻辑由 lib/visionImage.test.ts 覆盖。
vi.mock('../lib/visionImage', () => ({ downscaleForVision: async (d: string) => ({ dataUrl: d, mime: 'image/jpeg' }) }))
vi.mock('../lib/api', () => ({
...
describe('ChatPage 线程消息气泡渲染（防字段漂移：按 kind 渲染内容）', () => {
  ...
  it('图片消息「用 AI 描述」→ 调 visionDescribe 并显示描述（低视力家人网页端也能听懂图片，与 iOS 对齐）', async () => {
    ...
    // 送端点前经 downscaleForVision 降采样为 JPEG（故 mime=image/jpeg，非原 png）——大图不再被 413 拒。
    // 泛描述：question(第4参)为 undefined（有值才是 VQA）；首轮无历史 → history(第5参)为 undefined。
    expect(api.visionDescribe).toHaveBeenCalledWith('data:image/png;base64,AAAA', 'image/jpeg', expect.anything(), undefined, undefined)
  })
  ...
  it('图片消息「问」→ 把具体问题作为 question 参数发给 visionDescribe（图像问答 VQA）并显示答案', async () => {
    ...
    expect(api.visionDescribe).toHaveBeenCalledWith('data:image/png;base64,AAAA', 'image/jpeg', expect.anything(), '上面的电话号码是多少', undefined)
  })
  ...
  it('连续追问（对话式 VQA）：第二问把首轮 Q&A 作为 history 送上，模型据上下文答追问（对标 Be My AI）', async () => {
    ...
    expect(api.visionDescribe).toHaveBeenLastCalledWith('data:image/png;base64,AAAA', 'image/jpeg', expect.anything(), '多少钱',
