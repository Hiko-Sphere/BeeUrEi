...
/// 联系人当前位置的可读地址（GET /api/locations/address；仅境内高德有数据）。含**最近路口**（两条相交路名，iter382）
/// 与**最近地标**（如"国贸大厦"）——盲人转告出租/路人的强定位锚点（中式定位习惯常靠地标"到X大厦"），与本人「我在哪」
/// （WhereAmIComposer）同款；此前联系人侧丢弃了服务端已下发的 intersection/landmark（死字段），此处全兑现。
