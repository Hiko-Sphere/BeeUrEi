...
struct LiveLocationView: View {
    ...
    @MainActor private func loadAndSpeakAddress(_ c: ContactLocationInfo) async {
        ...
        if let info, let text = LiveLocationStrings.contactAddressText(address: info.address, township: info.township, aoiName: info.aoi?.name, aoiDistanceMeters: info.aoi?.distanceMeters, firstRoad: info.intersection?.firstRoad, secondRoad: info.intersection?.secondRoad, intersectionDirection: info.intersection?.direction, intersectionDistanceMeters: info.intersection?.distanceMeters, landmarkName: info.landmark?.name, landmarkDirection: info.landmark?.direction, landmarkDistanceMeters: info.landmark?.distanceMeters, lang) {
