...
export function registerProductRoutes(app: FastifyInstance): void {
  ...
  app.get('/api/product/:barcode', {
    ...
    if (outcome.kind === 'found') {
      ...
      cache.set(barcode, { name, allergens, traces, nutriScore: nutriScore ?? null, novaGroup: novaGroup ?? null, dietaryLabels, quantity, nutrientLevels, ingredients, energyKcal100g: energyKcal100g ?? null, macros100g, servingGrams: servingGrams ?? null, at: now })
    ...
    cache.set(barcode, { name: null, allergens: [], traces: [], nutriScore: null, novaGroup: null, dietaryLabels: [], quantity: '', nutrientLevels: {}, ingredients: '', energyKcal100g: null, macros100g: EMPTY_MACROS, servingGrams: null, at: now }) // notFound：可长缓存（真未收录短期不会变）
