...
const I18N = {
  zh: {
    ...
    helpRequests: '协助求助', helpClaimed: '已接起', helpNoneClaimedHint: '无人接起——请检查志愿者供给/推送',
    ...
    amapNet: '高德连接失败', amapNetHint: '连不上高德（超时/网络/DNS/防火墙），非 key/配额问题——尤其当「高德导航失败」为 0 却仍熔断时，多半是本机到高德的**出网不通**，请查服务器出站网络/DNS/防火墙，而非 key',
    amapDown: '导航骨干熔断中', amapDownVal: '断路·此刻不可用', amapDownHint: '高德连续失败已触发熔断，导航/周边/公交此刻全在快失败——请立即检查 key/配额/上游状态，冷却后自动探测复位',
    amapProbe: '导航骨干探测恢复', amapProbeVal: '探测中', amapProbeHint: '熔断冷却结束，正用下一次请求探测高德是否恢复；成功即自动恢复正常',
    ...
    visionQuotaHit: 'AI 视觉撞配额', visionQuotaHitHint: '累计撞每日上限的次数——频繁则配额可能过紧（尤其对话式追问增大用量后），可在「控制」页即时调高「AI 视觉每日配额」，无需重新部署',
    ...
    alwaysOn: '始终开启',
    featLabels: { messaging: '消息', calls: '远程协助通话', helpRequests: '公开求助', groups: '群组', familyLinks: '亲友绑定', mediaUpload: '媒体上传', navigation: '步行导航', locationSharing: '位置共享', sceneScan: '看一看（场景识别）', aiDescribe: 'AI 场景描述', emergency: '紧急报警', blocks: '拉黑', reports: '举报' },
    featDescs: { messaging: '私聊/群聊发送消息', calls: '发起远程协助音视频呼叫', helpRequests: '发起/认领公开求助', groups: '创建群组、管理成员', familyLinks: '绑定亲友/协助者', mediaUpload: '上传图片/视频', navigation: '步行路径导航', locationSharing: '亲友/协助者与盲人实时互相可见位置', sceneScan: '端侧场景识别（仅客户端隐藏）', aiDescribe: '云端视觉大模型描述照片/图像问答（需配 VISION_* 才生效）' },
    // 用户编辑（Admin v4）
  ...
  en: {
    ...
    helpRequests: 'Help requests', helpClaimed: 'Claimed', helpNoneClaimedHint: 'none claimed — check helper supply/push',
    ...
    amapNet: 'AMap connectivity failures', amapNetHint: "Can't reach AMap (timeouts/network/DNS/firewall) — not a key/quota issue. If this is nonzero while “AMap navigation errors” is 0 yet the breaker tripped, it's a connectivity problem: check the server's outbound network/DNS/firewall, not the key",
    amapDown: 'Navigation backbone tripped', amapDownVal: 'OPEN · down now', amapDownHint: 'AMap failed repeatedly and the circuit breaker is OPEN — navigation/nearby/transit are all fast-failing right now; check key/quota/upstream immediately, it auto-probes after cooldown',
    amapProbe: 'Navigation backbone probing', amapProbeVal: 'half-open', amapProbeHint: 'Cooldown elapsed; the next request probes whether AMap recovered — it auto-restores on success',
    ...
    visionQuotaHit: 'AI vision quota hits', visionQuotaHitHint: 'Times the per-user daily cap was hit — if frequent the quota may be too tight (especially now conversational follow-ups increase usage); raise “AI vision daily quota” on the Controls page in real time, no redeploy needed',
    ...
    alwaysOn: 'Always on',
    featLabels: { messaging: 'Messaging', calls: 'Remote-assist calls', helpRequests: 'Public help', groups: 'Groups', familyLinks: 'Family links', mediaUpload: 'Media upload', navigation: 'Walking navigation', locationSharing: 'Location sharing', sceneScan: 'Look (scene scan)', aiDescribe: 'AI scene description', emergency: 'Emergency alerts', blocks: 'Blocking', reports: 'Reporting' },
    featDescs: { messaging: 'Send 1:1 / group messages', calls: 'Start remote-assist calls', helpRequests: 'Start / claim public help', groups: 'Create groups, manage members', familyLinks: 'Link family / helpers', mediaUpload: 'Upload images / video', navigation: 'Walking route guidance', locationSharing: 'Real-time location sharing between contacts', sceneScan: 'On-device scene scan (client-hidden)', aiDescribe: 'Cloud vision-model image description / visual Q&A (needs VISION_* configured)' },
...
function renderDashboard() {
  ...
  viewEl().innerHTML = `
    ...
    <div class="cards">
      ...
      ${o.help != null && o.help.requests > 0 ? statCard(t('helpRequests'), o.help.requests, t('helpClaimed') + ': ' + o.help.claims + (o.help.claims === 0 ? ' · ' + t('helpNoneClaimedHint') : ''), o.help.claims === 0 ? 'danger' : '') : ''}
      ...
      ${o.amap != null && ((o.amap.timeouts || 0) + (o.amap.netErrors || 0)) > 0 ? statCard(t('amapNet'), (o.amap.timeouts || 0) + (o.amap.netErrors || 0), t('amapNetHint'), 'danger') : ''}
      ${o.amap != null && o.amap.breakerState === 'open' ? statCard(t('amapDown'), t('amapDownVal'), t('amapDownHint'), 'danger') : ''}
      ${o.amap != null && o.amap.breakerState === 'halfOpen' ? statCard(t('amapProbe'), t('amapProbeVal'), t('amapProbeHint')) : ''}
      ...
      ${o.vision != null && o.vision.quotaExceeded > 0 ? statCard(t('visionQuotaHit'), o.vision.quotaExceeded, t('visionQuotaHitHint')) : ''}
...
async function openUserDrawer(uid) {
  ...
  try {
    ...
    const callsHTML = (d.recentCalls || []).map((c) => `<div class="mini">${esc(t('dir')[c.direction] || c.direction)} · ${esc(c.peerName)} · <span class="text-dim">${esc(t('callStatus')[c.status] || c.status)}</span>${c.emergency ? ` <span class="pill danger">🆘 ${esc(t('sosCall'))}</span>` : ''}<span class="when">${esc(fmtDate(c.createdAt))}</span></div>`).join('') || `<div class="empty pad"><p>${esc(t('noCalls'))}</p></div>`;
...
const FEATURE_ORDER = ['messaging', 'calls', 'helpRequests', 'groups', 'familyLinks', 'mediaUpload', 'navigation', 'locationSharing', 'sceneScan', 'aiDescribe'];
