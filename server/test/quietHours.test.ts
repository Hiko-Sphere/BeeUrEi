...
describe('notifyUser 勿扰门（软通知抑制推送但仍持久化；紧急恒推送）', () => {
  ...
  it('勿扰时段内应急投递自测(delivery_check)：也始终推送——自测须反映真实应急路径（越勿扰，用户拍板）', () => {
    const { store, web } = seed(windowCoveringNow(true))
    const push = new RecPush()
    notifyUser(store, push, 'u1', 'delivery_check', 'T', 'B')
    expect(push.alerts).toHaveLength(1) // 勿扰时段也真送达（此前受勿扰约束 → 0，会让自测测不到真实路径）
    expect(web.sent).toHaveLength(1)
  })
