...
interface SpaTest {
  state: { lang: string; emergencies: unknown[]; calls: unknown[]; callsQuery: string; blocks: unknown[]; blocksQuery: string; links: unknown[]; linksQuery: string; overview: unknown; reports: unknown[]; token: string | null; bootCommit: string | null; updateReady: boolean }
  t: (k: string) => string
...
function loadSpa(): SpaTest {
  ...
  src = src.replace(anchor, '\nglobalThis.__test = { state, t, emergencySection, statCard, renderCalls, renderDashboard, renderReports, renderBlocks, renderLinks, pickWsToken, emergenciesCsvRows, emergencyTriageSort, openReportRepeatCounts, reportsTriageSort, settledField, trackServerCommit, validateFilterTerms };\nrender();')
  const noop = (): void => {}
...
describe('紧急事件值守分诊排序 emergencyTriageSort（待介入浮顶，不改存储序）', () => {
  ...
  it('同为进行中：无人响应(未 ack) 浮于 有人响应(已 ack) 之上——待介入优先于"有人在处理"，纵使前者更老', () => {
    const spa = loadSpa()
    const ackedNewer = ev({ at: 5_000, ackedAt: 5_100 })   // 有人已响应、较新
    const unackedOlder = ev({ at: 3_000 })                 // 无人响应、较老（默认无 ackedAt/escalatedAt）
    // 纯时间序会把 acked-newer 排前、埋掉待介入的 unacked；分诊须把"没人确认"的浮顶。
    expect(spa.emergencyTriageSort([ackedNewer, unackedOlder]).map((e) => e.at)).toEqual([3_000, 5_000])
  })
...
describe('管理面板 通话/拉黑行 主叫被叫·拉黑双方直达用户抽屉（审 SOS 通话/拉黑纠纷一键查全貌）', () => {
  it('通话行主叫、被叫都渲染成可点按钮，data-uid 指向各自用户 id（此前纯文本，无法从 SOS 通话点进主叫档案）', () => {
    const spa = loadSpa()
    spa.state.lang = 'zh'
    spa.state.callsQuery = ''
    spa.state.calls = [{ id: 'c1', callId: 'k1', callerId: 'blind1', callerName: '小明', calleeId: 'helper1', calleeName: '阿华', status: 'ended', emergency: true, createdAt: 1_700_000_000_000 }]
    spa.renderCalls()
    const html = spa.view.innerHTML
    expect(html).toMatch(/class="emerg-user row-user"[^>]*data-uid="blind1"/) // 主叫（SOS 求助者）→ 直达
    expect(html).toMatch(/class="emerg-user row-user"[^>]*data-uid="helper1"/) // 被叫 → 直达
    expect(html).toContain('小明')
    expect(html).toContain('阿华')
  })
  it('拉黑行拉黑方、被拉黑方都渲染成可点按钮，data-uid 指向各自用户 id', () => {
    const spa = loadSpa()
    spa.state.lang = 'en'
    spa.state.blocks = [{ id: 'b1', blockerId: 'u1', blockerName: 'Alice', blockedId: 'u2', blockedName: 'Bob', createdAt: 1_700_000_000_000 }]
    spa.renderBlocks()
    const html = spa.view.innerHTML
    expect(html).toMatch(/class="emerg-user row-user"[^>]*data-uid="u1"/)
    expect(html).toMatch(/class="emerg-user row-user"[^>]*data-uid="u2"/)
    expect(html).toContain('Alice')
    expect(html).toContain('Bob')
    expect(html).toContain('Open user detail') // rowOpenUser 提示（en）
  })
  it('id 做输出编码：畸形 callerId 的引号被 esc 中和，无法破出 data-uid 属性', () => {
    const spa = loadSpa()
    spa.state.lang = 'zh'
    spa.state.callsQuery = ''
    spa.state.calls = [{ id: 'c1', callId: 'k1', callerId: '"><b>x', callerName: 'x', calleeId: 'u2', calleeName: 'y', status: 'ended', createdAt: 1_700_000_000_000 }]
    spa.renderCalls()
    const html = spa.view.innerHTML
    expect(html).not.toContain('data-uid=""><b>x') // 未破出属性
    expect(html).toContain('&quot;&gt;&lt;b&gt;x') // 已被 HTML 实体编码中和
  })
  it('绑定关系行 owner、member 都渲染成可点按钮，data-uid 指向各自用户 id（核查紧急联系人设置一键查全貌）', () => {
    const spa = loadSpa()
    spa.state.lang = 'zh'
    spa.state.linksQuery = ''
    spa.state.links = [{ ownerId: 'blindA', ownerName: '小明', ownerRole: 'blind', memberId: 'helperB', memberName: '女儿', memberRole: 'family', relation: '女儿', isEmergency: true, status: 'accepted', createdAt: 1_700_000_000_000 }]
    spa.renderLinks()
    const html = spa.view.innerHTML
    expect(html).toMatch(/class="emerg-user row-user"[^>]*data-uid="blindA"/) // 被绑定的盲人 → 直达
    expect(html).toMatch(/class="emerg-user row-user"[^>]*data-uid="helperB"/) // 亲友 → 直达
    expect(html).toContain('小明')
    expect(html).toContain('女儿')
  })
})
