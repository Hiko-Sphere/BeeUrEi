...
declare module 'fastify' {
  interface FastifyInstance {
    ...
    metrics: Metrics
    callControl: CallControlBridge // 供优雅关闭时置 shuttingDown（信令层据此在部署重启中不广播 peer-left）
  }
}
...
export function buildApp(store: Store = makeDefaultStore(), options: AppOptions = {}): FastifyInstance {
  ...
  app.decorate('callControl', callControl) // 供优雅关闭置 shuttingDown（信令层据此在部署重启中不广播 peer-left）
  ...
  app.register(async (instance) => {
    ...
    registerRecoveryRoutes(instance, store, codes, mailer, codeSend, pushSender, callControl) // 找回密码（D1）；改密安全预警本人 + 踢在线 /ws
    registerAccountRoutes(instance, store, codes, mailer, appleVerifier, codeSend, pushSender, callControl) // 账号安全变更预警本人 + 撤销会话踢在线 /ws
    ...
    registerMedicalRoutes(instance, store, pushSender) // 紧急医疗信息（本人加密填写；紧急亲友遇险时按需拉取；查看即通知本人）
  ...
  // img-src 含 blob:：KYC 证件图经带鉴权 fetch → URL.createObjectURL(blob:) 渲染（不能用 <img src=API> 因需 Bearer）；
  // CSP 的 'self' **不覆盖 blob:**（浏览器按 scheme 匹配），缺 blob: 会静默拦掉证件图、审核员看不到证件无法核验。
  const ADMIN_CSP = "default-src 'self'; img-src 'self' data: blob:; style-src 'self'; script-src 'self'; "
