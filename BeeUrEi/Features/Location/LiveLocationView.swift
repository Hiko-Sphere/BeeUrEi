...
struct LiveLocationView: View {
    ...
    @MainActor private func loadAndSpeakAddress(_ c: ContactLocationInfo) async {
        ...
        if let info, let text = LiveLocationStrings.contactAddressText(address: info.address, township: info.township, aoiName: info.aoi?.name, firstRoad: info.intersection?.firstRoad, secondRoad: info.intersection?.secondRoad, lang) {
