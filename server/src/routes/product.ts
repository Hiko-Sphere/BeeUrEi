...
export function registerProductRoutes(app: FastifyInstance): void {
  const cache = new Map<string, { name: string | null; allergens: string[]; traces: string[]; nutriScore: string | null; novaGroup: number | null; at: number }>()
  ...
  app.get('/api/product/:barcode', {
    ...
    if (hit && now - hit.at < TTL) {
      return hit.name ? { name: hit.name, allergens: hit.allergens, traces: hit.traces, nutriScore: hit.nutriScore, novaGroup: hit.novaGroup } : reply.code(404).send({ error: 'not_found' })
    }
    ...
    if (outcome.kind === 'found') {
      const { name, allergens, traces, nutriScore, novaGroup } = outcome.info
      cache.set(barcode, { name, allergens, traces, nutriScore: nutriScore ?? null, novaGroup: novaGroup ?? null, at: now })
      return { name, allergens, traces, nutriScore: nutriScore ?? null, novaGroup: novaGroup ?? null }
    }
    cache.set(barcode, { name: null, allergens: [], traces: [], nutriScore: null, novaGroup: null, at: now }) // notFound：可长缓存（真未收录短期不会变）
