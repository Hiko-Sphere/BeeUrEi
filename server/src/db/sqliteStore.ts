...
export class SqliteStore implements Store {
  ...
  constructor(path: string) {
    ...
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS messages (
        id TEXT PRIMARY KEY, fromId TEXT, toId TEXT, kind TEXT, text TEXT, createdAt INTEGER, readAt INTEGER, reaction TEXT, groupId TEXT, editedAt INTEGER, replyTo TEXT);
  ...
  createMessage(m: ChatMessage): void {
    this.db.prepare('INSERT OR REPLACE INTO messages (id, fromId, toId, kind, text, createdAt, readAt, reaction, groupId, editedAt, replyTo) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
      .run(m.id, m.fromId, m.toId, m.kind, m.text, m.createdAt, m.readAt ?? null, m.reaction ?? null, m.groupId ?? null, m.editedAt ?? null, m.replyTo ?? null)
  }
