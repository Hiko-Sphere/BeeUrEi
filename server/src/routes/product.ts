...
export function registerProductRoutes(app: FastifyInstance): void {
  ...
  const TTL = 24 * 3600_000 // 商品名基本不变，缓存一天
  const MAX = 5000
  ...
  app.get('/api/product/:barcode', {
    ...
    if (hit && now - hit.at < TTL) {
      return hit.name ? { name: hit.name } : reply.code(404).send({ error: 'not_found' })
    }
    ...
    cache.set(barcode, { name: info?.name ?? null, at: now })
    return info ? { name: info.name } : reply.code(404).send({ error: 'not_found' })
  })
}
