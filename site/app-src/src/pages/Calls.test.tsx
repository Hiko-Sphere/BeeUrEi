...
describe('CallsPage 通话记录"加载更多"（silent cap 修复：不再只见最近 100 条）', () => {
  ...
  it('hasMore 时显示"加载更多"；点击→用最早一条为游标翻页并追加；到底后按钮消失', async () => {
    ...
    // 焦点接力：焦点落在首条**新**记录行**本身**（否则到底时按钮卸载、焦点丢到 body，读屏用户失位）。
    // 断言活动元素是该行 li（含新项 o1、不含首屏 n1）——不可只查 textContent.includes，因 body 含全部文本会假通过。
    await waitFor(() => {
      const active = document.activeElement as HTMLElement
      expect(active.tagName).toBe('LI')
      expect(active.textContent).toContain('联系人o1')
      expect(active.textContent).not.toContain('联系人n1')
    })
  })
