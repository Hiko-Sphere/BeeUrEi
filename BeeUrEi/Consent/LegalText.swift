...
• 位置数据——导航与「我在哪」在本地使用精确定位，但不上传精确坐标。当你发起求助或紧急呼叫时，可附带「省/市/区」级粗略位置帮助对方了解你大概在哪。目的：步行导航与让协助者/紧急联系人知道你大致方位。合法性基础：分享粗略位置属于同意（Art 6(1)(a)）；在紧急情形下，处理可基于你或他人的重大利益（Art 6(1)(d)）。接收方：为提供导航，必要坐标会发送给地图/天气服务商（见第 6 节）——海外用 Apple 地图、中国大陆用高德（AMap）、天气用 Open-Meteo（仅降精度坐标，无身份信息）；粗略求助位置会展示给你选定的协助者或紧急联系人。此外，若你已授予定位权限，通话录制可能附带录制设备的粗略位置作为元数据（见第 4 节）；我们不会为录制单独请求定位权限。
