...
final class AppRoute {
    ...
    enum FramingChannel { case banknote, countCash, scan, fullPage, bus, people, light, text, color, colorMatch, dates, phone, email, sceneAI }
    ...
    /// savedRoute=语音"走X路线"：按说出的名字模糊匹配已保存路线并沿信标执行（SavedRouteMatcher，歧义/无匹配则读出全部路线名）。
    enum NavAction: Equatable { case search(String), coordinate(lat: Double, lon: Double, name: String), backtrack, savedRoute(String) }
    ...
    /// 导航中**即将到来的转向**的按需回述文本（"下一步：还有约120米，左转进入建国路"），由 NavigationViewModel 每次更新
    /// 时写入、结束/开始导航清空。供语音"下一步怎么走"随时预听——beacon 只给方位、currentNavRemaining 只给总剩余，都不含
    /// "下个路口左还是右、还有多远"；经此共享值中转（导航屏是独立 sheet、其 model 不被 Hub 直接持有）。nil = 未在导航或尚未算出。
    var currentNavNextManeuver: String?
