...
public struct HazardCatalog: Sendable {
    ...
    public static let defaultHighRisk: Set<String> = [
        ...
        // 椅子/盆栽/餐桌：同一判据——沿街咖啡座椅、店门口花箱/盆栽、餐桌是最常见的人行道齐腰高固定障碍，盲人正面撞上真实伤害；
        // COCO chair/potted plant/dining table 本就检出命名（localizedName 产出"椅子"/"盆栽"/"餐桌"），此前漏高危加成，与 bench 不一致。
        // 餐桌尤甚：硬边齐腰、比椅子更易撞伤（与已收录的"椅子"成餐饮场景一对，缺一不可）。沙发(couch)偏软、伤害弱，暂不收。
    ...
    public static let defaultHighRiskEnglish: Set<String> = [
        ...
        "chair", "potted plant", "table", // 沿街座椅/花箱/餐桌：与 bench 同类的齐腰高固定障碍（zh 椅子/盆栽/餐桌 对齐；dining table→"table"）
    ]
