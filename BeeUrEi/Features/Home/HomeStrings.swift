...
enum HomeStrings {
    ...
    static func voiceCommandsHelp(_ l: Language) -> String {
        l == .zh
            ? "你可以说：开始导盲、带我去某地、坐公交去某地、回家、去公司、看一看、读一下文字、看看保质期、读电话号码、读邮箱、找我的钥匙、最近的厕所在哪、我在哪、周围有什么、我朝哪个方向、这是什么颜色、这两件搭不搭、天气、现在几点、还有多少电、今天几号、给某人发消息、给某人打电话、把位置发给某人、读一下消息、打开消息、打开设置、再说一遍、说慢点、说快点、说简短点、说详细点。需要人帮忙说\u{201C}求助\u{201D}；遇到危险说\u{201C}救命\u{201D}，会倒计时通知你的全部亲友。"
            : "You can say: start guide, take me to a place, take transit to a place, take me home, go to work, look, read this, check the expiry date, read a phone number, read an email, find my keys, where's the nearest restroom, where am I, what's around, which way am I facing, what color is this, do these two match, weather, what time is it, battery level, what's the date, send a message, call a family member by name, share my location with someone, read my messages, open messages, open settings, repeat, speak slower/faster, or ask for less/more detail. Say \u{201C}get help\u{201D} to reach anyone available; say \u{201C}emergency\u{201D} if you're in danger — it counts down and alerts all your contacts."
    }
