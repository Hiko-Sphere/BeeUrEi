...
**质量：** 核心 293 测 / 应用层 36 测 / 后端 159 测全过；覆盖率 后端91%+ 核心97%+；CI 绿。
**E5 余量：** LoginView/AuthGateView（~98条，登录链路）· AssistHomeView（协助者侧 ~71条）·
完整安全须知 DisclaimerText 英文版（法律文本需校对）· AdminHomeView/DevOverlay（内部工具，不列入）。
