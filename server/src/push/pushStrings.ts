...
export const pushStrings = {
  ...
  securityNotice: (event: SecurityEvent, l: PushLang): { title: string; body: string } => {
    ...
    switch (event) {
      ...
      case 'admin_identifier_changed': return { title: en ? 'Sign-in details changed by an administrator' : '管理员更改了你的登录信息',
        body: en ? 'An administrator changed your account sign-in details (email, phone or username). If you didn’t request this, contact us immediately.' : '管理员更改了你的账号登录标识（邮箱 / 手机号 / 用户名）。若非你本人请求，请立即联系我们并检查账号安全。' }
