...
enum LegalText {
    ...
    static let effectiveDate = "2026-07-13"
...
本隐私政策说明我们处理哪些个人数据、为何处理、与谁共享、保留多久，以及你对自己数据享有的权利。我们的核心承诺很简单：摄像头画面默认不离开你的手机；只有在你于通话中主动按住「显示画面」时，画面才会实时传给你自己选定的对方。通话仅在双方明确同意时才会被录制；为安全与防滥用，管理员仅在向通话双方实时告知（不可关闭的横幅 + 盲人侧语音）之后，方可对进行中的通话进行监管——这些都是公开告知、绝非隐蔽的处理，详见第 4 节。
...
2. 端侧处理原则（画面不上云）
端侧优先是 BeeUrEi 的基本设计。以下视觉 AI 推理都在你的 iPhone 本地完成，默认不上传：避障、识别物体、读字、识别纸币、扫码、辨色、找物、「周围的人」、光线探测。

相机画面与深度数据主要用于本地实时分析；上述端侧功能默认不上传服务器、不留存、不用于训练任何模型。

画面离开你的设备只有两种情况，且都由你主动逐次发起：

（一）远程协助：当你在通话中主动按住「显示画面」时，且仅在此期间，画面才会实时传给你自己选定的对方，用于让对方看到你眼前的场景以提供帮助。你松开后即停止传输。

（二）「AI 描述」与对图片提问（可选功能，仅在该功能已启用时可用）：当你主动点按「AI 描述」或对某张图片提问时，仅那一张经降采样的图片（连同你输入的问题，如有）会经我们的服务器发往一家第三方 AI 视觉模型服务商，用于生成对画面的文字描述或直接回答你的问题。我们不留存该图片、不用于训练；该服务商依其条款处理这一次请求。你不使用此功能时，不会有任何画面因此离开设备。
...
• 安全与审核数据——拉黑、举报记录（举报可附通话录制作为证据，见第 4 节）、管理审计日志，以及管理员对实时通话的监管记录。为保障安全、保护脆弱用户并防止滥用，管理员可对进行中的通话进行旁观（看到 / 听到）、在通话中开麦说话或强制结束——此时通话双方均会被实时告知（不可关闭的横幅 + 盲人侧语音播报，见第 4.2 节）。目的：维护社区安全、防止骚扰与滥用、处理举报与滥用调查。合法性基础：保障平台与用户安全的正当利益（Art 6(1)(f)），并以对通话双方的实时告知为前提。接收方：仅我们经授权的管理员 / 审核人员及 AWS（托管），不与任何第三方处理者共享。

• 通知——站内通知收件箱（例如你提交的、或涉及你的举报的处理结果）。目的：可靠地把与你相关的事件结果告知你。合法性基础：合同必要（Art 6(1)(b)）与维护安全治理的正当利益（Art 6(1)(f)）。接收方：仅我们及 AWS（托管）；若你已开启系统推送，提醒横幅经 Apple APNs 投递。
...
• AI 视觉描述数据（可选，仅当「AI 描述」功能已启用且你主动使用时）——你主动选择的那一张经降采样的图片，以及你输入的问题（如有）。目的：由第三方 AI 视觉模型对该图片生成文字描述或回答你的问题，让你"听懂"眼前或收到的图片。合法性基础：同意（Art 6(1)(a)），由你逐次主动发起。接收方：第三方 AI 视觉模型服务商（该次请求经我们自托管的后端转发；我们不留存该图片、不用于训练，见第 2 节）。若该功能未在你的部署中启用，则此功能不可用、不会发送任何图片。
...
4. 通话录制与管理员监管
本节说明两类涉及通话双方音视频的处理——通话录制，以及管理员对实时通话的合规监管。我们对其同意、告知与透明度作出特别约定。

4.1 通话录制。通话默认不录制。仅在通话双方均明确同意后方可开始录制；同意由被录方经鉴权接口亲自授予、由服务端权威核验，发起方无法伪造对方的同意。任一方均可在录制进行中将其终止；你也可随时拒绝录制，或撤回对今后录制的同意——撤回不影响撤回前已在同意基础上完成的录制的合法性。

录制的内容与元数据。经同意的录制会捕获该次通话所呈现的音频与视频（因而包含通话对方）。我们为每条录制保存元数据：录制时间、参与者、通话时长、录制原因，以及——仅当你已授予定位权限时——录制设备的粗略位置；我们不会为录制而单独弹出新的定位授权请求。

回看与访问。发起录制的一方可在 App 内回看自己的录制。为保障安全、防止滥用与处理举报，管理员可在保留期内访问录制（见 4.3）。

删除与合规留存。你可随时从自己的列表中删除自己的录制。出于安全并防止在调查期间销毁证据，被你删除的录制会对你隐藏，但在保留期届满前仍可能由管理员访问（合规留存 / legal hold）；保留期届满后，录制将由自动清理程序彻底删除。被某条尚未处理完毕的举报引用为证据的录制，将保留至该举报处理完毕。

作为举报证据。录制的拥有者可将自己的录制作为证据附加到一次滥用举报中；该录制随后会提供给我们的管理员 / 审核人员审阅（见 4.3 与第 6 节）。

留存期。录制按保留期自动删除，默认 7 天（管理员可在合理范围内配置该保留期）。

合法性基础。进行录制以同意为基础（Art 6(1)(a)）；管理员为安全、滥用防范与举报处理而访问录制、以及上述合规留存，以我们维护平台与用户安全的正当利益为基础（Art 6(1)(f)）。

4.2 管理员对实时通话的监管（旁观）。为保障安全、保护脆弱用户并防止滥用，管理员在必要时可对一通进行中的通话进行监管：旁观（看到 / 听到通话的音视频）、在通话中开麦说话，以及在必要时强制结束通话。

这绝非隐蔽监听。当管理员开始旁观时，通话双方都会被实时告知——界面会显示一条不可关闭的提示横幅，盲人侧还会有语音播报。管理员的旁观通道与 1:1 通话本身在传输上相互隔离；该能力仅在通话双方的客户端都支持并据此告知时方可启用（对无法显示该告知的旧版客户端，监管不会启用）。合法性基础：以我们维护平台与用户安全、防范滥用的正当利益为基础（Art 6(1)(f)），并以对通话双方的实时告知为前提。

4.3 接收方。录制内容与实时通话旁观仅在我们自托管的后端（部署于 AWS）内处理，访问限于我们经授权的管理员 / 审核人员，并留有管理审计记录；不与任何第三方处理者共享。
...
Version 3.0　·　Effective 2026-06-14
...
This Privacy Policy explains what personal data we process, why, with whom we share it, how long we keep it, and the rights you have over your data. Our core commitment is simple: your camera frames do not leave your phone by default; video is shared in real time only when you actively press and hold "Show video" during a call, and only with the person you yourself have chosen. Calls are recorded only with both parties' explicit consent; and, for safety and abuse prevention, an administrator may supervise an in-progress call only after both parties are notified in real time (a non-dismissible banner and a spoken announcement on the blind user's side) — these are openly disclosed, never covert, and are described in Section 4.
...
On-device-first is fundamental to BeeUrEi's design. The following vision AI inference runs locally on your iPhone and is not uploaded by default: obstacle avoidance, object recognition, text reading, banknote recognition, barcode scanning, color identification, finding objects, "people nearby," and light detection.

Camera frames and depth data are used primarily for local, real-time analysis. For the on-device features above, by default they are not uploaded to any server, not retained, and not used to train any model.

Camera frames leave your device only in two cases, each of which you start yourself, one action at a time:

(1) Remote assistance: when, and only while, you actively press and hold "Show video" during a call, your camera feed is streamed in real time to the person you yourself have chosen, so they can see the scene in front of you and help. Streaming stops the moment you release.

(2) AI Describe and asking about a photo (an optional feature, available only when it is enabled): when you actively tap "AI Describe" or ask a question about a photo, only that single downscaled image (together with your typed question, if any) is sent — via our server — to a third-party AI vision-model provider to generate a text description of the scene or answer your question. We do not retain that image or use it for training; the provider processes that one request under its own terms. When you do not use this feature, no camera frames leave your device because of it.
...
• Safety and moderation data — block and report records (a report may have a call recording attached as evidence, see Section 4), administrative audit logs, and records of administrative supervision of live calls. To safeguard safety, protect vulnerable users, and prevent abuse, administrators may observe an in-progress call (see and hear it), speak into it, or force-end it — in which case both parties are notified in real time (a non-dismissible banner and a spoken announcement on the blind user's side, see Section 4.2). Purpose: keep the community safe, prevent harassment and abuse, and handle reports and abuse investigations. Legal basis: our legitimate interest in the safety of the platform and its users (Art 6(1)(f)), conditioned on real-time notice to both parties. Recipients: our authorized administrators/moderators only and AWS (hosting); not shared with any third-party processor.

• Notifications — an in-app notification inbox (for example, the outcome of a report you filed or that concerns you). Purpose: to reliably tell you the outcome of events that concern you. Legal basis: performance of a contract (Art 6(1)(b)) and our legitimate interest in safety governance (Art 6(1)(f)). Recipients: us only and AWS (hosting); if you have enabled system push, the alert banner is delivered via Apple APNs.

• Recordings — see Section 4 (including the captured audio and video; the time/location/participants/duration metadata; playback; deletion and legal hold; use as report evidence; and administrative supervision). Legal basis: consent (Art 6(1)(a)) for making a recording; our legitimate interest (Art 6(1)(f)) for administrators' access and the legal hold for safety/abuse-prevention/report-handling. Recipients: recordings are not shared with any third-party processor; they are stored only on our self-hosted backend (deployed on AWS) and may be accessed by our authorized administrators/moderators as described in Section 4.
...
• AI vision-description data (optional, only when the "AI Describe" feature is enabled and you actively use it) — the single downscaled image you choose, plus your typed question (if any). Purpose: to have a third-party AI vision model describe that image or answer your question, so you can "understand" a scene or a photo you received. Legal basis: consent (Art 6(1)(a)), started by you one action at a time. Recipients: a third-party AI vision-model provider (the request is forwarded via our self-hosted backend; we do not retain the image or use it for training, see Section 2). If this feature is not enabled on your deployment, it is unavailable and no image is sent.
...
4. Call recording and administrative supervision
This section covers two kinds of processing that involve both parties' audio and video — call recording, and administrative supervision of live calls. We make specific commitments here about consent, notice, and transparency.

4.1 Call recording. Calls are not recorded by default. Recording can begin only after both parties to the call have given explicit consent; consent is given by the recorded party in person through an authenticated endpoint and verified authoritatively by the server, so the initiating party cannot fake the other party's consent. Either party may stop an in-progress recording; you may also decline recording at any time, or withdraw your consent to future recording — withdrawal does not affect the lawfulness of recording already made with consent before withdrawal.

What a recording captures, and its metadata. A consented recording captures the call's audio and video as presented (and therefore includes the other participant). For each recording we keep metadata: the time of recording, the participants, the call duration, the reason for recording, and — only if you have already granted location permission — an approximate location of the recording device; we do not prompt for new location permission just to record.

Playback and access. The party who started a recording can play their own recording back in the app. To safeguard safety, prevent abuse, and handle reports, administrators may access recordings within the retention period (see 4.3).

Deletion and legal hold. You may delete your own recording from your own list at any time. For safety and to prevent the destruction of evidence during an investigation, a recording you delete is hidden from you but may remain accessible to administrators until the retention period expires (a legal hold); after the retention period, recordings are permanently deleted by an automatic sweep. A recording cited as evidence by a report that has not yet been resolved is kept until that report is resolved.

As report evidence. The owner of a recording may attach their own recording as evidence to an abuse report; the recording is then made available to our administrators/moderators for review (see 4.3 and Section 6).

Retention. Recordings are automatically deleted on expiry; the default retention is 7 days (an administrator may configure this retention period within reasonable limits).

Legal basis. Making a recording rests on consent (Art 6(1)(a)); administrators' access to recordings for safety, abuse prevention, and report handling, and the legal hold above, rest on our legitimate interest in the safety of the platform and its users (Art 6(1)(f)).

4.2 Administrative supervision of live calls (observation). To safeguard safety, protect vulnerable users, and prevent abuse, administrators may, where necessary, supervise an in-progress call: observe it (see and hear its audio and video), speak into the call, and, where necessary, force-end it.

This is never covert. When an administrator begins observing, both parties to the call are notified in real time — the interface shows a non-dismissible banner, and the blind user's side also hears a spoken announcement. The administrator's observation channel is isolated in transit from the 1:1 call itself; this capability can be enabled only when both parties' clients support it and surface that notice (for older clients that cannot display the notice, supervision is not enabled). Legal basis: our legitimate interest in the safety of the platform and its users and in preventing abuse (Art 6(1)(f)), conditioned on real-time notice to both parties.

4.3 Recipients. Recording content and live-call observation are processed only on our self-hosted backend (deployed on AWS); access is limited to our authorized administrators/moderators and is logged in an administrative audit trail; they are not shared with any third-party processor.
...
视觉 AI 推理默认在你的 iPhone 本地完成（可选的「AI 描述」除外——使用时那一张图会经服务器发往第三方视觉模型，详见隐私政策第 2 节）。本 App 持续迭代，我们可在不实质削弱核心无障碍功能的前提下增改功能。本服务现阶段免费提供，且依其开源许可不得被商用收费。
...
安全监管：为保障安全、保护脆弱用户并防止滥用，我们还可在向通话双方实时告知的前提下，对进行中的通话进行监管——包括旁观（看到 / 听到）、在通话中说话，以及在必要时强制结束通话；此类监管绝非隐蔽，发生时双方都会看到不可关闭的提示横幅、盲人侧另有语音播报。此外，录制的拥有者可将自己的录制作为证据附加到举报中供我们审核。上述处理的具体方式、告知机制与合法性基础，详见《隐私政策》第 4 节。
...
9. 隐私
我们把隐私当作安全的一部分来设计。视觉 AI 推理默认在你的 iPhone 本地完成；上述端侧功能的相机画面与深度数据默认不会上传至或存储于我们的服务器，也不用于训练。摄像头画面除可选的「AI 描述」（使用时那一张图经我们的服务器发往第三方 AI 视觉模型，不留存、不训练，详见隐私政策第 2 节）外，离开你设备的情形，是在通话中实时、端到端地传给你选定的对方，且仅在你主动按住"显示画面"期间——在无法建立直连时，该传输会经我们自托管的 TURN 中继转发。
...
Effective date: 2026-06-14　·　Version 3.0
...
Vision AI inference runs locally on your iPhone by default (except the optional AI Describe, whose single image is sent via our server to a third-party vision model when used — see Section 2 of the Privacy Policy). The App evolves over time, and we may add or change features provided we do not materially diminish core accessibility functions. The Service is currently provided free of charge and, under its open-source license, may not be commercialized for a fee.
...
Safety supervision: to safeguard safety, protect vulnerable users, and prevent abuse, we may also supervise an in-progress call — including observing it (seeing and hearing it), speaking into it, and, where necessary, force-ending it — provided that both parties to the call are notified in real time. Such supervision is never covert; when it happens, both parties see a non-dismissible banner and the blind user's side also hears a spoken announcement. In addition, the owner of a recording may attach their own recording as evidence to a report for our review. The detailed means, the notice mechanism, and the legal basis for this processing are set out in Section 4 of the Privacy Policy.
...
9. Privacy
We treat privacy as part of safety by design. Vision AI inference runs locally on your iPhone by default; for those on-device features, camera frames and depth data are, by default, not uploaded to or stored on our servers and are not used for training. Apart from the optional AI Describe (when used, that single image is sent via our server to a third-party AI vision model — not retained or used for training, see Section 2 of the Privacy Policy), camera video leaves your device only during a call, in real time and end-to-end with the person you choose, and only while you actively press and hold "Show video" — where a direct peer connection is unavailable, that transmission is relayed via our self-hosted TURN relay.
...
通话录制：通话默认不予录制；仅在通话双方明确同意时方可录制，录制将按《隐私政策》所载保留期（默认 7 天）自动删除。
...
端侧优先：本 App 的视觉 AI 推理（避障、识别物体、读文字、识别纸币、扫码、辨色、找物、"周围的人"、光线探测）默认在 iPhone 本地完成。相机画面与深度数据主要用于本地实时分析，默认不上传、不留存、不用于训练。画面离开设备只有两种由你主动发起的情形：① 在通话中主动按住"显示画面"时实时传给你自己选定的对方；② 可选的「AI 描述」——那一张图经服务器发往第三方视觉模型（详见隐私政策）。
...
远程协助：协助者 / 志愿者为独立个人，其建议仅供参考；提供者不对其行为或建议作担保，亦不承担责任。请勿在通话中透露密码、验证码等敏感信息。通话默认不予录制；仅在通话双方明确同意时方可录制，录制将按《隐私政策》所载保留期（默认 7 天）自动删除。
...
Version: 3.0 · Effective date: 2026-06-14.
...
Call recording: calls are not recorded by default; recording is permitted only with the express consent of both parties to the call, and recordings are automatically deleted after the retention period stated in the Privacy Policy (7 days by default).
...
On-device first: the App's vision AI inference (obstacle avoidance, object/text/banknote recognition, barcode scanning, color detection, find-my-things, "people nearby", light detection) runs locally on the iPhone by default. Camera frames and depth data are used primarily for local, real-time analysis; by default they are not uploaded, retained, or used for training. Camera frames leave the device only in two cases you start yourself: (1) during a call, streamed in real time to someone you select while you actively press and hold "Show Video"; and (2) the optional AI Describe — that single image is sent to a third-party vision model (see the Privacy Policy).
