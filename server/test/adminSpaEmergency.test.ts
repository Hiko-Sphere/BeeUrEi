...
interface SpaTest {
  state: { lang: string; emergencies: unknown[]; calls: unknown[]; callsQuery: string; blocks: unknown[]; blocksQuery: string; overview: unknown; reports: unknown[]; token: string | null; bootCommit: string | null; updateReady: boolean }
  t: (k: string) => string
...
function loadSpa(): SpaTest {
  ...
  src = src.replace(anchor, '\nglobalThis.__test = { state, t, emergencySection, statCard, renderCalls, renderDashboard, renderReports, renderBlocks, pickWsToken, emergenciesCsvRows, emergencyTriageSort, openReportRepeatCounts, reportsTriageSort, settledField, trackServerCommit, validateFilterTerms };\nrender();')
  const noop = (): void => {}
