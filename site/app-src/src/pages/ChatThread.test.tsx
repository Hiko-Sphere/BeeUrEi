...
describe('ChatPage 线程消息气泡渲染（防字段漂移：按 kind 渲染内容）', () => {
  ...
  it('图片消息「用 AI 描述」→ 调 visionDescribe 并显示描述（低视力家人网页端也能听懂图片，与 iOS 对齐）', async () => {
    ...
    // 泛描述：question(第4参)为 undefined（有值才是 VQA）。
    expect(api.visionDescribe).toHaveBeenCalledWith('data:image/png;base64,AAAA', 'image/png', expect.anything(), undefined)
  })
  ...
  it('图片消息「问」→ 把具体问题作为 question 参数发给 visionDescribe（图像问答 VQA）并显示答案', async () => {
    ...
    // 第 4 参数=用户问题（VQA）；空问题不触发（按钮 disabled），此处非空。
    expect(api.visionDescribe).toHaveBeenCalledWith('data:image/png;base64,AAAA', 'image/png', expect.anything(), '上面的电话号码是多少')
  })
