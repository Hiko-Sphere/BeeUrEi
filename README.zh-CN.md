...
<p align="center">
  ...
  <sub>端侧实时避障 · 步行导航 · 场景与物体识别 · 远程真人协助。免费、隐私、可自托管。</sub>
</p>
...
**BeeUrEi**（Be Your Eye）是一款原生 iOS App：用 iPhone 主摄像头 + LiDAR，为视力障碍人士提供四种核心能力——**零额外硬件、零订阅、视觉 AI 端侧运行。**

- 🛡️ **实时避障** —— 端侧 AI 连续判断前方障碍，用语音 + AirPods 双耳空间音 + 震动提示「是什么、几点钟方向、还有多远」；含地面落差/台阶边缘检测、红绿灯三通道提示（节奏音 + 节奏震动 + 全屏高对比色块）、接近声呐。
- 🧭 **步行导航** —— 空间音「信标」指路 + 转向播报 + 路名 callout；**面包屑原路返回**（记下走过的路，一键带回出发点）；环境感知三键「我在哪 / 周围有什么 / 前方有什么」（时钟方位 + 米数）。
- 📷 **场景与物体识别** —— 对准即说：识别物品 / 朗读文字 / **读整页文档（多页连读）** / **识别纸币** / **扫码认商品** / 找我的东西（教它认你的物品）/ 找周围的物品 / **周围的人**（人数·方位·距离，不识别身份）/ 公交线路 / 光线探测 / 照片触摸探索 / 识别历史回放——全部端侧，画面不上云。
- 🤝 **远程真人协助** —— 一键视频呼叫亲友/志愿者；视障侧画面**默认不外发**，按需才显示；来电铃声 + 震动 + 语音报来电人；通话中亲友可远程开手电/变焦帮你看清。
...
| **端口–适配器** | 安全逻辑下沉为平台无关、可单测的核心包；I/O（相机/ARKit/语音/定位/网络）协议化、可注入。 |
| **可自托管** | 后端 + WebRTC 信令 + TURN 全可自托管，零第三方按量费用，数据完全归你。 |
...
```sh
xcodegen generate            # 改完 project.yml 后重新生成 BeeUrEi.xcodeproj
open BeeUrEi.xcodeproj
```
在 Xcode 选中 target → **Signing & Capabilities** 选你的 Apple ID → 接真机 → `⌘R`。
（相机/LiDAR 必须真机；模拟器会显示「设备不支持」。）

> 真实 WebRTC 媒体使用本地 vendored 的 `Frameworks/WebRTC.xcframework`（已 gitignore，约 91MB）。放入后即用真实引擎；未放入时 App 仍可编译，通话会如实失败（不伪装连接）。
...
Docker / TURN / APNs 推送：`docker build -t beeurei-api server/`，以 `--env-file server/.env` 运行，前置自选隧道。
...
- 经**多轮多智能体对抗式代码审查**，累计修复 140+ 个真实缺陷（含信令窃听、避障距离/方向错算、深色地面落差误报、到达判定绕过精度门控、磁干扰信标指错、中断后空间音永久失声、语音通道互相淹没、盲人侧错误无声、缺少二次确认等）并全部补齐回归测试。
...
- 全程 **VoiceOver** 可用；开启时语音自动改走无障碍公告，不与 VoiceOver 抢话；**Magic Tap** 直达各屏高频操作。
...
- **免责告知**：首次完整知情同意（中英双语，盲人会被朗读全文）+ 每次开始一句可关的简短提醒。
...
- 远程视频走 **P2P**；视障侧画面**默认不外发**，按需才显示；新对端接入自动回到不发送。
...
| 阶段 | 内容 | 状态 |
|---|---|---|
| 核心安全逻辑 | 60+ 模块 / 319 测试 / 多轮对抗式审查 | ✅ |
| Phase 1 实时避障 | LiDAR 深度 + 检测 + 落差/红绿灯 + 空间音 | ✅（待真机调参）|
...
| Phase 3 远程视频 | 信令 + 隐私门控 + 来电铃/CallKit + WebRTC 媒体 | ✅（待双真机验证）|
...
本项目以 **[PolyForm Noncommercial License 1.0.0](LICENSE)** 发布：你可以出于任何**非商业**目的自由地**使用、学习、修改、分发与共同制作**（个人、研究、教育、公益与无障碍用途），完全免费，也欢迎协作并分享你的修改。但**不得用于商业目的**（出售，或向它本应服务的视障用户收费）。这是一个公益项目；商业授权请联系 Hiko Sphere 彦穹科技。

> 本软件按「现状」提供，不附带任何明示或暗示的担保。它是辅助工具，**不能替代白手杖、导盲犬或定向行走训练**。
