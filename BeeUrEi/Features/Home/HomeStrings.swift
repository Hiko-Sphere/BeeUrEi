...
enum HomeStrings {
    ...
    /// 语音"结束导航"反馈：已停 / 当前没在导航（盲人无从看屏，须语音确认命令是否生效）。
    static func navStoppedSpeak(_ l: Language) -> String { l == .zh ? "已结束导航。" : "Navigation stopped." }
    static func navNotActiveSpeak(_ l: Language) -> String { l == .zh ? "现在没有在导航。" : "You're not navigating right now." }
    /// 「还有多远」但导航刚开始/定位未定、尚未算出剩余里程：如实说正在计算，别静默也别误报"没在导航"。
    static func navRemainingComputing(_ l: Language) -> String { l == .zh ? "正在计算剩余里程，请稍候再问。" : "Still calculating the distance left — ask again in a moment." }
    /// 「带我去那里」但还没找过最近地点：提示先找（如"最近的厕所在哪"），别静默——盲人无从知道命令为何没反应。
    static func noPlaceToNavigateSpeak(_ l: Language) -> String {
        l == .zh ? "还没有刚找到的地点。先说一句，比如\u{201C}最近的厕所在哪\u{201D}，再说\u{201C}带我过去\u{201D}。"
                 : "No place found yet. Ask for one first — say \u{201C}where's the nearest restroom\u{201D}, then \u{201C}take me there\u{201D}."
    }

    static func voiceCommandsHelp(_ l: Language) -> String {
        l == .zh
            ? "你可以说：开始导盲、带我去某地、坐公交去某地、走某条路线、结束导航、还有多远、下一步怎么走、原路返回、回家、去公司、看一看、描述场景、读一下文字、读整页、看看保质期、读电话号码、读邮箱、认一下钱、数一叠钱、扫个码、这是几路车、找我的钥匙、找空座位、最近的厕所在哪、带我去那里、我在哪、周围有什么、前方有什么、有没有人、我朝哪个方向、这是什么颜色、这两件搭不搭、光线怎么样、天气、现在几点、还有多少电、今天几号、给某人发消息、给某人打电话、把位置发给某人、读一下消息、打开消息、打开设置、再说一遍、说慢点、说快点、说简短点、说详细点。设了安全报到后说\u{201C}报平安\u{201D}即可结束报到。需要人帮忙说\u{201C}求助\u{201D}；遇到危险说\u{201C}救命\u{201D}，会倒计时通知你的全部亲友。"
            : "You can say: start guide, take me to a place, take transit to a place, follow a saved route, stop navigation, how much farther, what's my next turn, retrace my steps, take me home, go to work, look, describe the scene, read this, read the whole page, check the expiry date, read a phone number, read an email, identify money, count my cash, scan a code, which bus is this, find my keys, find an empty seat, where's the nearest restroom, take me there, where am I, what's around, what's ahead, who's there, which way am I facing, what color is this, do these two match, how bright is it, weather, what time is it, battery level, what's the date, send a message, call a family member by name, share my location with someone, read my messages, open messages, open settings, repeat, speak slower/faster, or ask for less/more detail. With a safety check-in running, say \u{201C}I'm safe\u{201D} to end it. Say \u{201C}get help\u{201D} to reach anyone available; say \u{201C}emergency\u{201D} if you're in danger — it counts down and alerts all your contacts."
    }
