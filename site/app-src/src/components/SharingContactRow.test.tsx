...
describe('SharingContactRow 共享位置联系人行（定位/呼叫/发消息）', () => {
  ...
  it('查看地址：带最近地标时附"最近地标X"（中式定位常靠地标转告）；与AOI重名则不重复', async () => {
    vi.spyOn(api, 'contactAddress').mockResolvedValue({
      address: '光华路5号', township: '', landmark: { name: '国贸大厦', direction: '东', distanceMeters: 120 },
    })
    render(<ul><SharingContactRow c={c} lang="zh" t={t} live onLocate={() => {}} onCall={() => {}} onMessage={() => {}} /></ul>)
    fireEvent.click(screen.getByRole('button', { name: '查看 小明 所在地址' }))
    expect(await screen.findByText(/光华路5号，最近地标国贸大厦/)).toBeInTheDocument()
  })

  it('查看地址：地标名与AOI重名 → 不重复附（避免"在国贸一带…最近地标国贸"赘述）', async () => {
    vi.spyOn(api, 'contactAddress').mockResolvedValue({
      address: '光华路5号', township: '', aoi: { name: '国贸中心', distanceMeters: 0 }, landmark: { name: '国贸中心', direction: '', distanceMeters: 0 },
    })
    render(<ul><SharingContactRow c={c} lang="zh" t={t} live onLocate={() => {}} onCall={() => {}} onMessage={() => {}} /></ul>)
    fireEvent.click(screen.getByRole('button', { name: '查看 小明 所在地址' }))
    const el = await screen.findByText(/光华路5号（在国贸中心一带）/)
    expect(el.textContent).not.toMatch(/最近地标/) // 重名 → 无地标子句
  })
