...
- [x] iOS：AirPods 头部追踪 `HeadTracker`（Q8，增强空间音；核心 `BeaconDirection.relative` 已测；`SpatialAudioFeedback.setListenerYaw`；无耳机回退）——已升级为**真正双耳 HRTF 渲染**（选可用最优算法，核心 `SpatialAudioPolicy` 已测）+ 头偏航零位标定（`HeadYawReference` 已测，消除开机偏置与断连跳变）
